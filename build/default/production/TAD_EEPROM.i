...
void EEPROM_Init(void) {
    totalLogs = readByte(0xFF);
    if (totalLogs > 15) totalLogs = 0;
}
...
void EEPROM_DemanaEsborrarLogs(void) {
    for (unsigned char addr = 0; addr < (14 * 15); addr++) {
        writeByte(addr, 0xFF);
    }
    totalLogs = 0;
    writeByte(0xFF, 0);
...
void EEPROM_DemanaLlegirLogs(void) {
    if (pendingReadRequest == 0) {
        pendingReadRequest = 1;
    }
}

void EEPROM_Motor(void) {
    static unsigned char estatGlobal = 0;
    ...
    if (estatGlobal == 1) {
        switch (eepromSubState) {
            ...
            case 1:
                if (jLog < 14) {
                    log[jLog] = readByte(baseAddr + (iLog * 14) + jLog);
                    jLog++;
                } else {
                    jLog = 0;
                    sendIndex = 0;
        ...
        switch (eepromSubState) {
            ...
            case 1:
                if (idxChar < 14) {
                    unsigned char from = 0x00 + ((idxDesplac + 1) * 14) + idxChar;
                    unsigned char to = 0x00 + (idxDesplac * 14) + idxChar;
            ...
            case 3:
                if (idxLog < 14) {
                    unsigned char addr = 0x00 + (totalLogs * 14) + idxLog;
            ...
            case 4:
                totalLogs++;
                if (totalLogs > 15) totalLogs = 15;
                writeByte(0xFF, totalLogs);
                idxLog = 0;
                desantLog = 0;
                estatGlobal = 0;
                break;
