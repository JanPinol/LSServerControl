...
void SIS_ControlTemperatura(void) {
    ...
    switch (state) {
        ...
        case 1:
            if (TI_GetTics(timerTemperatura) >= 1000 * tempsMostreig) {
                ADCON0bits.CHS = 2;
                ADCON0bits.GO = 1;
                state = 2;
            }
...
void parsejaInitialize(char *comanda) {
    ...
    any = (comanda[i] - '0') * 10 + (comanda[i+1] - '0'); i += 3;
    mes = (comanda[i] - '0') * 10 + (comanda[i+1] - '0'); i += 3;
    dia = (comanda[i] - '0') * 10 + (comanda[i+1] - '0'); i += 3;
...
char identificaComanda(char *comanda) {
    if (iniciaAmb(comanda, "INITIALIZE:")) return CMD_ID_INITIALIZE;
    if (iniciaAmb(comanda, "SET_TIME:")) return CMD_ID_SET_TIME;
    if (iniciaAmb(comanda, "GET_LOGS")) return CMD_ID_GET_LOGS;
    if (iniciaAmb(comanda, "GET_GRAPH")) return CMD_ID_GET_GRAPH;
    if (iniciaAmb(comanda, "RESET")) return CMD_ID_RESET;
